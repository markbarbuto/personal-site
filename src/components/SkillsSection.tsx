        ...
        {skillGroups.map((group, index) => (
          <div
            ...
            className={`grid gap-x-3 gap-y-[18px] py-[22px] sm:grid-cols-[132px_1fr] sm:items-center ${
              index === skillGroups.length - 1 ? "" : "border-b border-black/5"
            }`}
          >
            <h3 className="text-[13px] font-semibold uppercase tracking-[0.04em] text-[#a1a1a6]">
              {group.label}
            </h3>
            <div className="flex flex-wrap gap-2.5">
              {group.skills.map((skill) => (
                <span
                  ...
                  className="glossy-tile inline-flex items-center gap-[9px] rounded-full border border-black/5 bg-panel py-2 pl-[11px] pr-[15px] text-sm font-medium"
                >
